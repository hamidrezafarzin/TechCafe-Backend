...
class Branch:
    ...
    def get_bulk_count(self, branch, rangeFrom, rangeTo):
        data = {"branch": branch, "rangeFrom": rangeFrom, "rangeTo": rangeTo}
        result = self.client.service.GetBulkCount(**self.get_data(), **data)
        return result

    def get_bulk_receptions(self, bulkId, fromRows):
        data = {"bulkId": bulkId, "fromRows": fromRows}
        result = self.client.service.GetBulkReceptions(**self.get_data(), **data)
        return result
    ...
    def send_multiple_sms(self, to, _from, text, isflash, udh):
        data = {"to": to, "from": _from, "text": text, "isflash": isflash, "udh": udh}
        result = None
        if isinstance(_from, list):
            result = self.client.service.SendMultipleSMS2(**self.get_data(), **data)
        else:
            result = self.client.service.SendMultipleSMS(**self.get_data(), **data)
        return result

    def update_bulk_delivery(self, bulkId):
        data = {"bulkId": bulkId}
        result = self.client.service.UpdateBulkDelivery(**self.get_data(), **data)
        return result
