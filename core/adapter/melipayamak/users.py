...
class Users:
    ...
    def add_complete(self, options):
        result = self.client.service.AddUserComplete(**self.get_data(), **options)
        return result

    def add_with_location(self, options):
        result = self.client.service.AddUserWithLocation(**self.get_data(), **options)
        return result
    ...
    def get_transactions(self, targetUsername, creditType, dateFrom, dateTo, keyword):
        ...
        result = self.client.service.GetUserTransactions(**self.get_data(), **data)
        return result
